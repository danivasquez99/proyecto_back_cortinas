...
public class ProductController {
    ...
    public ResponseEntity<LinkedList<Product>> getAllProducts(@RequestParam String email,@RequestParam String password) {
    ...
    public ResponseEntity<Product> getProductById(@PathVariable int id, @RequestParam String email,@RequestParam String password) {
        User user = authService.authenticate(email, password);
        if (!authService.hasPermission(user, "PRODUCT_VIEW")) {
            return ResponseEntity.status(403).build();
        }
